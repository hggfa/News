...
class StorageAdapter {
    ...
    load() {
        ...
        try {
            ...
            console.error(`Échec du chargement ${this.storageFile}!`, error);
            this.data = null;
        }
    }
    ...
    save() {
        if (this.data === null) {
            throw new Error('Les données doivent encore être chargées');
        }

        try {
            fse.writeJSONSync(this.storageFile, this.data);
        } catch (error) {
            console.error(`Impossible d'enregistrer les données dans ${this.storageFile}!`, error);
        }
    }
    ...
    get(key) {
        ...
        if (typeof key !== 'string') {
            throw new TypeError('la clé doit être une chaîne');
        }
