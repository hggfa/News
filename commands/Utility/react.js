...
exports.run = (bot, msg, args) => {
    if (args.length < 1) {
        throw 'Vous devez fournir un texte pour réagir avec.';
    }

    const fetchOptions = { limit: 1 };
    if (args[1]) {
        if (!/\d{18}/.test(args[1])) {
            throw `${args[1]} n'est pas un ID de message valide !`;
        }
    ...
    msg.channel.fetchMessages(fetchOptions).then(messages => {
        if (messages.length < 1) {
            return msg.error('Impossible de trouver le message.');
        }
...
exports.info = {
    ...
    description: 'Réagit avec le dernier message envoyé (ou un autre message) avec un texte donné (ne peut pas contenir d\'espaces)'
};
