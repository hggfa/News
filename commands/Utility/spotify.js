...
exports.run = async (bot, msg, args) => {
    if (args.length < 1) {
        throw 'Vous devez spécifier au moins un URI Spotify!';
    }
    ...
    case 'playlist':
        ...
        break;
    default:
        throw 'Désolé, je ne peux pas analyser ce type d\'URI.';
    }
...
function getArtistEmbed(msg, spotifyId, withPreview) {
    spotifyApi.getArtist(spotifyId)
        .then(data => {
            ...
            if (withPreview) msg.channel.send(`Vous avez Spotify ? Cliquez ci-dessous pour jouer ! ${apiResponse.external_urls.spotify}`);

        }).catch(err => {
            throw `Quelque chose s'est mal passé ! ${err}`;
        });
}
...
exports.info = {
    name: 'spotify',
    usage: 'spotify <url>',
    description: 'Analyse un spotri-uri et sort ses informations.',
    examples: [
    ...
    options: [
        {
            name: '-preview',
            description: 'Envoie un autre message où le discord place son propre lecteur intégré'
        }
    ]
};
