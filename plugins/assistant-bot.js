    ...
    init() {
        ...
        if (this.selfBot.config[AssistantBot.CONFIG_NAME]) {
            this.assistantBot.login(this.selfBot.config[AssistantBot.CONFIG_NAME])
                .catch(err => {
                    if (err.message === 'Des informations de connexion incorrectes ont été fournies.') {
                        console.error(`Jeton incorrect prévu pour ${AssistantBot.CONFIG_NAME}. S'il vous plaît lancé ${chalk.green('yarn run config')} pour le corriger.`);
                    }
                });
        }
    }
